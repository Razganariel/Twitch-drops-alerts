...
export const steamSchema = z.object({
  ...
  apiKey: z.string().length(32, "La clé API Steam fait 32 caractères").transform(stripHtml),
}).refine(
  (data) => data.steamId || data.username,
  { message: "Steam ID ou pseudo requis", path: ["steamId"] }
)
